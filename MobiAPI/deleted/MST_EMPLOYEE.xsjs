...
function getEmployee() {
	...
	try {
		...
		while (rEmployee.next()) {
			...
			record.POSITION_ID = rEmployee.getInteger(12);
			record.POSITION_VALUE = rEmployee.getString(13);
			record.Status = rEmployee.getInteger(14);
			record.UserType = "Employee";
			getStatusDesc(record);
			output.results.push(record);
		}
	...
	try {
		if (dataLine.length > 0) {
			for (var i = 0; i < dataLine.length; i++) {
				...
				pstmtCallPro.setInteger(12, parseInt(dicLine.POSITION_ID, 10));
